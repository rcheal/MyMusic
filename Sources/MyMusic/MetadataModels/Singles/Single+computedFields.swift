...
extension Single {
    ...
    var composerLastName: String? { lastName(composer) }

    var artistLastName: String? { lastName(artist) }

    var conductorLastName: String? { lastName(conductor) }

    func lastName(_ name: String?) -> String? {
        if var value = name {
            let pattern = " *\\(\\d{4}-\\d{4}\\) *"
